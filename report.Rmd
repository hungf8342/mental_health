...
set.seed(47)
model_full<-lm((suicides)~.,data=logtable_crop)
summary(model_full)
...
library(caret)
set.seed(35)
part<-sample(rownames(logtable_crop[logtable_crop$suicides>0,]),20)
locs.toUse<-logtable_crop[part,]
ggplot(locs.toUse,aes(x=socialRec.per.person,y=log(suicides)))+
  geom_point(aes(color=facility_cnt>quantile(logtable_crop$facility_cnt,0.5)))+
  geom_label_repel(aes(fill=facility_cnt>quantile(logtable_crop$facility_cnt,0.5),label=part),
                   fontface = 'bold', color = 'black', box.padding = 0.5, point.padding = 1, segment.color = 'grey50')+
  ...
  labs(title = "Suicides, Social Rec $, & Facility Density", y = "Log (suicide rates)", x = "Social Rec 
       Spending")
```
