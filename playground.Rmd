...
```{r}
set.seed(35)
part<-sample(rownames(logtable_crop_normalized[logtable_crop_normalized$suicides>0,]),60)
locs.toUse<-logtable_crop_normalized[part,]
ggplot(locs.toUse,aes(x=socialRec,y=log(suicides)))+geom_point(aes(color=facility_cnt>.4))+
  geom_label_repel(aes(fill=facility_cnt>.4,label=part),fontface = 'bold',     color = 'black', box.padding = 0.5, point.padding = 1, segment.color = 'grey50')+
