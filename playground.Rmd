...
# I used the one with top 50, instead of the one including cities with low search volume.
gtrends <- read_csv("~/math154/ma154-project5-teammentalhealth/gtrends_20042015_top50.csv")  %>%inner_join(cities_longlat,by="location")
...
logtable<-inner_join(citydem,gtrends,by="location") %>% inner_join(city_suis,by="location") %>% inner_join(citydem2,by="location") %>% mutate(suicides=suicides*100000/pop,healthcarepp=healthcarepp/pop,activities=activities/pop,socialRec=socialRec/pop,entertainment=entertainment/pop,presdrugs=presdrugs/pop,healthcarebiz=healthcarebiz*1000/pop)
...
```{r}
set.seed(47)
model<-lm((suicides)~.,data=logtable_crop)
summary(model)
```
...
```{r}
ggplot(logtable_crop,aes(x=hits,y=log(suicides)))+geom_point()
```
