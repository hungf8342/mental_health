...
logtable<-inner_join(citydem,cities_dep,by="location") %>% inner_join(city_suis,by="location") %>% inner_join(citydem2,by="location") %>% mutate(suicides=suicides*100000/pop,healthcarepp=healthcarepp/pop,activities=activities/pop,socialRec=socialRec/pop,entertainment=entertainment/pop,presdrugs=presdrugs/pop,healthcarebiz=healthcarebiz*1000/pop)
#logtable$hiRate<- ifelse(logtable$suicides>median(logtable$suicides),1,0)
logtable_crop<-logtable[,-c(1,23:27)]
norm_logtable<-logtable %>% select(c(nonfamily,presdrugs,asian,medIncome)) %>% sapply(.,as.numeric) %>% scale() %>% as.data.frame()
whole_norm_logtable<-logtable[,-c(1,23:27)] %>% sapply(.,as.numeric) %>% scale() %>% as.data.frame()
```

```{r}
set.seed(47)
model<-lm((suicides)~.,data=whole_norm_logtable)
summary(model)
```
